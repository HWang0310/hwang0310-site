...
2026-07-29 完成减少动态交互、经历路径、页眉头像、约数文案和发布记录隐私修复：

- 修复提交：`6221b299ba547671485319f3167dabe107914277`
- 预览部署 ID：`f261cf17-3b5a-486e-a9ae-dbd2f5959525`
- 预览不可变地址：<https://f261cf17.hwang0310-site.pages.dev>
- 生产部署 ID：`c344928d-9714-4975-ab55-83b76a41eb70`
- 生产不可变地址：<https://c344928d.hwang0310-site.pages.dev>
- 生产自定义域名：<https://hwang0310.dpdns.org/>

本轮验证结果：

- `npm test`：8 个测试文件、58 个测试全部通过。
- `npm run build`：通过，装配 102 个文件和四个报告日期。
- `npm run test:e2e`：25 个测试通过、5 个按浏览器项目配置跳过。
- 预览和生产均通过首页、论文、收入入口、四个报告日期、武汉跳转、GitHub、邮箱和隐私检查。
- 真实 Chromium 与 WebKit 均通过减少动态点击、桌面横向经历路径和手机纵向经历路径回归。

本次后续修复的回滚点为紧邻上一生产部署：

- 部署 ID：`4a447888-3118-47a9-90f8-12a0bd1e0480`
- 部署地址：<https://4a447888.hwang0310-site.pages.dev>
